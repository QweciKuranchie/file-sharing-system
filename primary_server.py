...
def handle_client(client_sock: socket.socket, addr: Tuple[str, int]) -> None:
    ...
    try:
        ...
        if parts[1] not in (TCP_CLIENT_SECRET, TCP_REPLICATION_SECRET):
            client_sock.sendall(b"ERROR UNAUTHORIZED\n")
            return
        ...
        while True:
            ...
            if cmd == 'PING':
                ...
                try:
                    ...
                    try:
                        import replication
                        replicated = replication.replicate_file(saved_name, filepath)
                ...
                try:
                    # Move to temporary name in a dedicated internal rollback directory to allow rollback on replication failure
                    rollback_dir = os.path.join(SHARED_FILES_DIR, ".tmp_rollback")
                    os.makedirs(rollback_dir, exist_ok=True)
                    import uuid
                    temp_filename = f"{uuid.uuid4().hex}_{filename}"
                    temp_filepath = os.path.join(rollback_dir, temp_filename)
                    ...
                    try:
                        import replication
                        outcome = replication.propagate_delete(filename)
                    except Exception as rep_err:
                        logger.error(f"Replication deletion failed for {filename}: {rep_err}")
                        outcome = replication.ReplicationOutcome.AMBIGUOUS
                    
                    if outcome == replication.ReplicationOutcome.SUCCESS:
                        os.remove(temp_filepath)
                        client_sock.sendall(b"OK FILE_DELETED\n")
                        logger.info(f"Deleted file and propagated deletion: {filename}")
                    elif outcome in (replication.ReplicationOutcome.REJECTED, replication.ReplicationOutcome.AMBIGUOUS):
                        # Rollback local delete
                        os.rename(temp_filepath, filepath)
                        client_sock.sendall(b"ERROR REPLICATION_FAILED\n")
                        logger.warning(f"Deletion failed with outcome {outcome}, rolled back delete: {filename}")
                ...
                try:
                    ...
                    try:
                        import replication
                        outcome = replication.propagate_rename(old_name, new_name)
                    except Exception as rep_err:
                    ...
                    if outcome == replication.ReplicationOutcome.SUCCESS:
                        ...
                        client_sock.sendall(b"ERROR REPLICATION_FAILED\n")
                        logger.warning(f"Rename propagation failed with outcome {outcome}, rolled back: '{old_name}'")
